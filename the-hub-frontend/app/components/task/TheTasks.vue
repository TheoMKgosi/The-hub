...
const handleMouseMove = (event: MouseEvent) => {
  ...
  if (longPressTimer.value && (Math.abs(event.movementX) > 5 || Math.abs(event.movementY) > 5)) {
    cancelLongPress()
  }
}
          ...
          >
            <template #item="{ element: task }">
              <div v-if="matchFilter(task)" :data-task-id="task.task_id">
                <div class="bg-surface-light dark:bg-surface-dark shadow-md rounded-lg p-4 border-l-4 hover:shadow-lg transition-all duration-200"
                  :class="[
                    task.status === 'complete' ? 'border-success' : 'border-warning',
                    isReorderMode && reorderTaskId === task.task_id ? 'ring-2 ring-primary shadow-lg scale-105' : '',
