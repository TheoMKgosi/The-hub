...
<template>
  <div class="space-y-3">
    ...
    <div v-if="!showForm" class="flex items-center justify-center">
      <UButton @click="toggleForm" variant="outline" size="sm" label="Add Task"
        class="w-full py-2.5 border-2 border-dashed border-surface-light/30 dark:border-surface-dark/30 hover:border-primary/50 hover:bg-surface-light/10 dark:hover:bg-surface-dark/10 transition-colors" />
    </div>
    ...
    <div v-else
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <UButton @click="toggleForm" variant="ghost" size="sm" icon="i-lucide-x" />
      </div>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="flex gap-2 pt-3 border-t border-surface-light/20 dark:border-surface-dark/20">
          <UButton type="submit" variant="outline" size="sm" :disabled="!formData.title.trim() || isSubmitting"
            class="flex-1" label="Add Task" />
          <UButton type="button" @click="toggleForm" variant="outline" size="sm" label="Cancel" />
        </div>
      </form>
    </div>
  </div>
