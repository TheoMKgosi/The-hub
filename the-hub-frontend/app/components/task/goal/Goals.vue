...
<template>
  <div class="px-6">
    ...
    <div
      ...
      <div class="flex flex-wrap gap-2 items-center mb-2">
        <div class="flex gap-2">
          <NavLink v-for="filterOption in ['all']" :key="filterOption" :active="filter === filterOption" variant="tab"
            @click="filter = filterOption">
            {{ filterOption.charAt(0).toUpperCase() + filterOption.slice(1) }}
          </NavLink>
        </div>
        <input v-model="searchQuery" placeholder="Search goals..."
          class="grow shadow-sm bg-surface-light dark:bg-surface-dark px-3 py-2 rounded-md border border-surface-light dark:border-surface-dark text-text-light dark:text-text-dark placeholder:text-text-light/50 dark:placeholder:text-text-dark/50 focus:outline-none focus:ring-2 focus:ring-primary w-full sm:w-0" />
      </div>
      <div>
        <FormGoal />
      </div>
    ...
    <div
      class="px-3 py-5 bg-surface-light/10 dark:bg-surface-dark/10 backdrop-blur-md shadow-sm mt-4 rounded-lg border border-surface-light/20 dark:border-surface-dark/20">
      <!-- Create Goal Form -->
      ...
      <template v-else>
        ...
        <div v-else-if="filteredGoals.length === 0" class="text-text-light dark:text-text-dark/60">No goals match your
          search</div>

        <div v-if="filteredGoals.length > 0"
          class="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-4 sm:mt-6">
          <div v-for="goal in filteredGoals" :key="goal.goal_id"
            ...
            <div v-if="editingGoalId !== goal.goal_id" class="flex flex-col h-full">
              ...
              <div class="mb-4 flex flex-wrap gap-2">
                <span v-if="goal.due_date" class="px-2 py-1 text-xs font-medium rounded-full" :class="new Date(goal.due_date) < new Date() && goal.status !== 'completed'
                ...
                <span class="px-2 py-1 text-xs font-medium rounded-full" :class="goal.status === 'completed'
              ...
              <!-- Goal Tasks Section -->
              <div class="mb-4">
                <USlideover title="Tasks" close>
                  <UButton label="View tasks" color="neutral" variant="subtle" />
                  <template #body>
              ...
              <!-- Add Task to Goal -->
              <div class="mb-4">
                <AddTaskToGoal :goal-id="goal.goal_id" :goal-title="goal.title" />
              </div>

              <!-- Goal Actions -->
              <div
                class="flex items-center justify-between mt-auto pt-4 border-t border-surface-light/20 dark:border-surface-dark/20">
                <span class="text-xs text-text-light dark:text-text-dark/60">
                  Double-click to edit
                </span>
                <div class="flex gap-2">
                  <UButton label="Edit" variant="outline" color="neutral" size="sm" @click="startEdit(goal)" />
                  <UButton label="Delete" variant="outline" color="error" size="sm" @click="deleteGoal(goal.goal_id)" />
                </div>
              </div>
            </div>
            ...
            <div v-else class="flex flex-col w-full space-y-4">
              ...
              <div class="flex gap-2 pt-2 border-t border-surface-light/20 dark:border-surface-dark/20">
                <UButton @click="cancelEdit" label="Cancel" variant="outline" color="neutral" size="md" />
                <UButton @click="saveEdit(goal.goal_id)" label="Save" variant="soft" color="neutral" size="md"
