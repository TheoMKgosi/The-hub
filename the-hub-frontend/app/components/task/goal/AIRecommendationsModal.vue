    ...
    <div class="p-4 overflow-y-auto max-h-[60vh]">
      ...
      <div v-if="loading" class="flex items-center justify-center py-12">
        <div class="flex flex-col items-center gap-3">
          ...
          <p class="text-sm text-text-light/60 dark:text-text-dark/60">Analyzing goal and generating
            recommendations...</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        <div v-for="(recommendation, index) in recommendations" :key="index"
          ...
          <div class="flex items-start justify-between gap-3">
            ...
            <UButton @click="handleAddTask(recommendation)" variant="outline" size="sm" label="Add"
              class="shrink-0" />
          </div>
        </div>
      </div>
