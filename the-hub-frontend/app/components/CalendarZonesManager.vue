...
import { useCalendarZonesStore } from '@/stores/calendar-zones'
...
<template>
  <div>
    <!-- Trigger Button -->
    <UiButton @click="openManager" variant="outline" size="sm">
    ...
    <Teleport to="body">
      <div v-if="showManager" class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center p-4 z-50" @click="closeManager">
        <div class="bg-surface-light dark:bg-surface-dark rounded-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto shadow-xl border border-surface-light/30 dark:border-surface-dark/30" @click.stop>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div class="lg:col-span-2">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <UiButton @click="showCreateForm = true" variant="primary" size="sm">
                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 102 0v-5a1 1 0 01-1-1H10zM9 3a1 1 0 00-1 1v5H3a1 1 0 00-1 1v5a1 1 0 001 1h5v5a1 1 0 102 0v-5a1 1 0 001-1v-5H9V4a1 1 0 00-1-1z" clip-rule="evenodd" />
                    </svg>
                    New Zone
                  </UiButton>
                </div>
                ...
                <div v-else-if="zonesStore.zones.length === 0" class="text-center py-8 text-text-light/60 dark:text-text-dark/60">
                  ...
                  <div v-for="zone in zonesStore.zones" :key="zone.id"
                    ...
                    <div class="flex items-center justify-between mb-3">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-sm text-text-light/60 dark:text-text-dark/60">{{ getCategoryInfo(zone.category).description }}</p>
                        </div>
                      </div>
                      <div class="flex items-center gap-2">
                        ...
                        <UiButton @click="toggleZone(zone)" variant="default" size="xs" :title="zone.is_active ? 'Deactivate' : 'Activate'">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-3">
                      ...
                      <div>
                        ...
                        <p class="text-sm text-text-light/80 dark:text-text-dark/80">{{ formatDaysOfWeek(zone.days_of_week) }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1">
                          ...
                          <span class="text-sm font-medium text-text-light dark:text-text-dark">{{ zone.priority }}/10</span>
                        </div>
                      ...
                      <div class="flex gap-2">
                        <UiButton @click="startEdit(zone)" variant="default" size="xs">Edit</UiButton>
                        <UiButton @click="deleteZone(zone.id)" variant="danger" size="xs">Delete</UiButton>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="lg:col-span-1">
                <div v-if="showCreateForm" class="bg-surface-light/10 dark:bg-surface-dark/10 rounded-lg p-4 border border-surface-light/30 dark:border-surface-dark/30">
                  ...
                  <form @submit.prevent="editingZone ? updateZone() : createZone()" class="space-y-4">
                    <div>
                      <label class="block text-sm font-medium mb-1 text-text-light dark:text-text-dark">Zone Name</label>
                      <input
                        v-model="newZone.name"
                        type="text"
                        placeholder="e.g., Work Hours, Class Time"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div>
                        <label class="block text-sm font-medium mb-1 text-text-light dark:text-text-dark">Start Time</label>
                        <input
                          v-model="newZone.start_time"
                          type="time"
                          class="w-full px-3 py-2 border border-surface-light/30 dark:border-surface-dark/30 bg-surface-light/20 dark:bg-surface-dark/20 text-text-light dark:text-text-dark rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                          required
                        />
                      ...
                      <div>
                        <label class="block text-sm font-medium mb-1 text-text-light dark:text-text-dark">End Time</label>
                        <input
                          v-model="newZone.end_time"
                          type="time"
                          class="w-full px-3 py-2 border border-surface-light/30 dark:border-surface-dark/30 bg-surface-light/20 dark:bg-surface-dark/20 text-text-light dark:text-text-dark rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                          required
                        />
                    ...
                    <div>
                      <label class="block text-sm font-medium mb-1 text-text-light dark:text-text-dark">Priority (1-10)</label>
                      <select
                        v-model="newZone.priority"
                        class="w-full px-3 py-2 border border-surface-light/30 dark:border-surface-dark/30 bg-surface-light/20 dark:bg-surface-dark/20 text-text-light dark:text-text-dark rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                      >
                    ...
                    <div class="flex gap-2">
                      ...
                      <UiButton @click="showCreateForm = false; editingZone = null; resetForm()" variant="default" size="sm">
                        Cancel
                      </UiButton>
                    </div>
                  </form>
                ...
                <!-- Zone Statistics -->
                <div v-else class="bg-surface-light/10 dark:bg-surface-dark/10 rounded-lg p-4 border border-surface-light/30 dark:border-surface-dark/30">
                  ...
                  <div class="space-y-3">
                    <div class="flex justify-between items-center">
                      ...
                      <span class="text-sm font-medium text-text-light dark:text-text-dark">{{ zonesStore.zones.length }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-sm text-text-light dark:text-text-dark">Active Zones</span>
                      <span class="text-sm font-medium text-green-600 dark:text-green-400">{{ zonesStore.zones.filter(z => z.is_active).length }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-sm text-text-light dark:text-text-dark">AI Scheduling Enabled</span>
                      <span class="text-sm font-medium text-blue-600 dark:text-blue-400">{{ zonesStore.zones.filter(z => z.allow_scheduling).length }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      ...
                      <span class="text-sm font-medium text-text-light dark:text-text-dark">
                        {{ zonesStore.zones.length > 0 ? (zonesStore.zones.reduce((sum, z) => sum + z.priority, 0) / zonesStore.zones.length).toFixed(1) : '0' }}/10
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="mt-4 pt-4 border-t border-surface-light/20 dark:border-surface-dark/20">
                    ...
                    <div class="space-y-1">
                      <div v-for="category in zonesStore.categories.slice(0, 5)" :key="category.name" class="flex justify-between items-center">
