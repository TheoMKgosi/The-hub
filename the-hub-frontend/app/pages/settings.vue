...
// Add tri-modal reactive state
const triModalEnabled = computed({
  get: () => userSettings.value.task?.['tri-modal'] || false,
  set: (value) => updateTriModalSetting(value)
})
// Add auto-save function
const updateTriModalSetting = async (enabled: boolean) => {
  try {
    // Update local state immediately
    userSettings.value = {
      ...userSettings.value,
      task: {
        ...userSettings.value.task,
        'tri-modal': enabled
      }
    }
    ...
    addToast('Tri-modal setting updated', 'success')
  } catch (error) {
    addToast('Failed to update tri-modal setting', 'error')
  }
}
</script>
...
<template>
  <div class="min-h-screen bg-background-light dark:bg-background-dark">
    <!-- Header -->
    <div class="bg-surface-light dark:bg-surface-dark border-b border-gray-200 dark:border-gray-700">
      <div class="max-w-4xl mx-auto px-6 py-6">
        <h1 class="text-3xl font-bold text-text-light dark:text-text-dark">Settings</h1>
      </div>
    </div>

    <!-- Tabs Navigation -->
    <div class="bg-surface-light dark:bg-surface-dark sticky top-0 z-10">
      <div class="max-w-4xl mx-auto px-6">
        <nav class="flex space-x-8" aria-label="Settings tabs">
          <button @click="activeTab = 'profile'" :class="[
            'py-4 px-4 font-medium text-sm transition-all relative',
            activeTab === 'profile'
              ? 'text-primary border border-gray-200 dark:border-gray-700 border-b-0 rounded-t-lg bg-surface-light dark:bg-surface-dark'
    ...
    <div class="max-w-4xl mx-auto p-6">
      ...
      <div v-show="activeTab === 'tasks'" class="space-y-6">
        <div class="space-y-4">
          <div class="flex items-center space-x-3">
            <input type="checkbox" :checked="triModalEnabled" @change="updateTriModalSetting($event.target.checked)"
              class="rounded border-gray-300 dark:border-gray-600 bg-surface-light dark:bg-surface-dark text-primary focus:ring-2 focus:ring-primary w-4 h-4" />
            <div class="flex-1">
              <label class="font-medium text-sm text-text-light dark:text-text-dark">
                Enable Tri-modal Task Management
              </label>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Organize tasks across Planning, Execution, and Analysis phases
              </p>
            </div>
          </div>
        </div>
