...
<template>
  <div class="bg-white shadow-md sticky">
    <div class="flex justify-between items-center p-4">
      ...
      <!-- Desktop Nav -->
      <div class="hidden md:flex items-center gap-4 bg-primary text-white rounded-2xl">
        <RouterLink v-for="item in labels" :key="item.link" :to="item.link" :class="[
    ...
    <div v-if="showMenu" class="md:hidden flex flex-col items-center gap-2 pb-4">
      <RouterLink v-for="item in labels" :key="item.link" :to="item.link" @click="showMenu = false" :class="[
        'w-full text-center p-2 rounded-md hover:bg-gray-200 transition',
        isActive(item.link) ? 'bg-gray-300 font-bold' : ''
      ]">
        {{ item.label }}
      </RouterLink>
      <button @click="logout" class="w-full p-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition">
        Logout
      </button>
    </div>
  </div>
