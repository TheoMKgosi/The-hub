...
func GetBudgetCategories(c *gin.Context) {
	...
	config.Logger.Sugar().Infof("Fetching budget categories for user ID: %s with order: %s", userIDUUID, orderClause)
	...
	if err := query.Find(&categories).Error; err != nil {
		config.Logger.Sugar().Errorf("Error fetching budget categories for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch categories"})
		return
	}

	config.Logger.Sugar().Infof("Found %d budget categories for user ID %s", len(categories), userIDUUID)
...
func GetBudgetCategory(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category ID param: %s", categoryIDStr)
	...
	config.Logger.Sugar().Infof("Fetching budget category ID: %s for user ID: %s", categoryID, userIDUUID)
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", categoryID, userIDUUID).First(&category).Error; err != nil {
		config.Logger.Sugar().Errorf("Budget category ID %s not found for user %s: %v", categoryID, userIDUUID, err)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget category not found"})
		return
	}

	config.Logger.Sugar().Infof("Successfully retrieved budget category ID %s for user %s", categoryID, userIDUUID)
...
func CreateBudgetCategory(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category input: %v", err)
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget category creation")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("name = ? AND user_id = ?", input.Name, userIDUUID).First(&existingCategory).Error; err == nil {
		config.Logger.Sugar().Warnf("Duplicate budget category name '%s' for user %s", input.Name, userIDUUID)
	...
	config.Logger.Sugar().Infof("Creating budget category for user %s: %s", userIDUUID, input.Name)
	if err := config.GetDB().Create(&category).Error; err != nil {
		config.Logger.Sugar().Errorf("Error creating budget category for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not create budget category"})
		return
	}

	config.Logger.Sugar().Infof("Successfully created budget category ID %s for user %s", category.ID, userIDUUID)
...
func UpdateBudgetCategory(c *gin.Context) {
	...
	categoryID, err := uuid.Parse(categoryIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category ID param for update: %s", categoryIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget category ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget category update")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", categoryID, userID).First(&category).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget category not found for update: ID %d, User %v", categoryID, userID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget category not found"})
		return
	}
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid update input for budget category ID %d: %v", categoryID, err)
	...
	if input.Name != nil {
		...
		if err := config.GetDB().Where("name = ? AND user_id = ? AND id != ?", *input.Name, userID, categoryID).First(&existingCategory).Error; err == nil {
			config.Logger.Sugar().Warnf("Duplicate budget category name '%s' for user %v during update", *input.Name, userID)
			c.JSON(http.StatusBadRequest, gin.H{"error": "Category name already exists"})
			return
		}
	...
	if len(updates) == 0 {
		config.Logger.Sugar().Warnf("No valid fields provided for budget category update: ID %d", categoryID)
		c.JSON(http.StatusBadRequest, gin.H{"error": "No valid fields to update"})
		return
	}

	config.Logger.Sugar().Infof("Updating budget category ID %d for user %v with data: %+v", categoryID, userID, updates)
	if err := config.GetDB().Model(&category).Updates(updates).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to update budget category ID %d: %v", categoryID, err)
	...
	if err := config.GetDB().First(&category, category.ID).Error; err != nil {
		config.Logger.Sugar().Errorf("Error retrieving updated budget category ID %d: %v", category.ID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not reload updated budget category"})
		return
	}

	config.Logger.Sugar().Infof("Successfully updated budget category ID %d for user %v", category.ID, userID)
...
func DeleteBudgetCategory(c *gin.Context) {
	...
	categoryID, err := uuid.Parse(categoryIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category ID param for delete: %s", categoryIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget category ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget category deletion")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", categoryID, userID).First(&category).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget category not found for delete: ID %d, User %v", categoryID, userID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget category not found"})
		return
	}

	config.Logger.Sugar().Infof("Deleting budget category ID %d for user %v", categoryID, userID)
	if err := config.GetDB().Delete(&category).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to delete budget category ID %d: %v", categoryID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete budget category"})
		return
	}

	config.Logger.Sugar().Infof("Successfully deleted budget category ID %d for user %v", categoryID, userID)
...
func SoftDeleteBudgetCategory(c *gin.Context) {
	...
	categoryID, err := uuid.Parse(categoryIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category ID param for soft delete: %s", categoryIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget category ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget category soft deletion")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", categoryID, userID).First(&category).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget category not found for soft delete: ID %d, User %v", categoryID, userID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget category not found"})
		return
	}
	...
	config.Logger.Sugar().Infof("Soft deleting budget category ID %d for user %v", categoryID, userID)
	if err := config.GetDB().Save(&category).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to soft delete budget category ID %d: %v", categoryID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete budget category"})
		return
	}

	config.Logger.Sugar().Infof("Successfully soft deleted budget category ID %d for user %v", categoryID, userID)
	c.JSON(http.StatusOK, gin.H{"message": "Budget category deleted successfully", "category": category})
}
...
func RestoreBudgetCategory(c *gin.Context) {
	...
	categoryID, err := uuid.Parse(categoryIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget category ID param for restore: %s", categoryIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget category ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget category restore")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", categoryID, userID).First(&category).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget category not found for restore: ID %d, User %v", categoryID, userID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget category not found"})
		return
	}
	...
	if category.DeletedAt.Time.IsZero() {
		config.Logger.Sugar().Warnf("Budget category ID %d is not soft deleted", categoryID)
	...
	config.Logger.Sugar().Infof("Restoring budget category ID %d for user %v", categoryID, userID)
	if err := config.GetDB().Save(&category).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to restore budget category ID %d: %v", categoryID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to restore budget category"})
		return
	}

	config.Logger.Sugar().Infof("Successfully restored budget category ID %d for user %v", categoryID, userID)
	c.JSON(http.StatusOK, category)
}
