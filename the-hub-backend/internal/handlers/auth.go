...
func RequestPasswordReset(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid forgot password input: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input", "details": err.Error()})
		return
	}

	config.Logger.Sugar().Infof("Password reset requested for email: %s", input.Email)
	...
	if err := config.GetDB().Where("email = ?", input.Email).First(&user).Error; err != nil {
		...
		config.Logger.Sugar().Warnf("Password reset requested for non-existent email: %s", input.Email)
	...
	if err != nil {
		config.Logger.Sugar().Errorf("Failed to generate reset token for user %s: %v", user.ID, err)
	...
	if err := config.GetDB().Create(&passwordResetToken).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to create password reset token for user %s: %v", user.ID, err)
	...
	if err := emailService.SendPasswordResetEmail(user.Email, resetToken); err != nil {
		config.Logger.Sugar().Errorf("Failed to send password reset email to %s: %v", user.Email, err)
		// Don't return error to user as this would reveal if email exists
	}

	config.Logger.Sugar().Infof("Password reset token created and email sent for user: %s", user.ID)
	c.JSON(http.StatusOK, ForgotPasswordResponse{
		Message: "If an account with that email exists, a password reset link has been sent.",
	})
}
...
func ResetPassword(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid reset password input: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input", "details": err.Error()})
		return
	}

	config.Logger.Sugar().Infof("Password reset attempt with token: %s", input.Token[:8]+"...")

	// Find the reset token
	var resetToken models.PasswordResetToken
	if err := config.GetDB().Where("token = ?", input.Token).First(&resetToken).Error; err != nil {
		config.Logger.Sugar().Warnf("Invalid reset token used: %s", input.Token[:8]+"...")
	...
	if !resetToken.IsValid() {
		if resetToken.Used {
			config.Logger.Sugar().Warnf("Attempted to use already used reset token: %s", input.Token[:8]+"...")
			c.JSON(http.StatusUnauthorized, gin.H{"error": "This reset token has already been used"})
			return
		}
		if resetToken.IsExpired() {
			config.Logger.Sugar().Warnf("Attempted to use expired reset token: %s", input.Token[:8]+"...")
	...
	if err != nil {
		config.Logger.Sugar().Errorf("Password hashing failed for user %s: %v", resetToken.UserID, err)
	...
	if err := config.GetDB().Model(&models.User{}).Where("id = ?", resetToken.UserID).Update("password", hashedPassword).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to update password for user %s: %v", resetToken.UserID, err)
	...
	if err := config.GetDB().Model(&resetToken).Update("used", true).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to mark reset token as used: %v", err)
		// Don't return error as password was already updated
	}

	config.Logger.Sugar().Infof("Password reset successfully for user: %s", resetToken.UserID)
...
func RefreshToken(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid refresh token input: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input", "details": err.Error()})
		return
	}

	config.Logger.Sugar().Infof("Token refresh attempt with token: %s", input.RefreshToken[:8]+"...")

	// Validate the refresh token
	refreshTokenRecord, err := util.ValidateRefreshToken(input.RefreshToken)
	if err != nil {
		if err == jwt.ErrTokenExpired {
			config.Logger.Sugar().Warnf("Attempted to use expired refresh token: %s", input.RefreshToken[:8]+"...")
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Refresh token has expired"})
			return
		}
		config.Logger.Sugar().Warnf("Invalid refresh token used: %s", input.RefreshToken[:8]+"...")
	...
	if err != nil {
		config.Logger.Sugar().Errorf("Access token generation failed for user ID %s: %v", refreshTokenRecord.UserID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate access token"})
		return
	}

	config.Logger.Sugar().Infof("Token refresh successful for user: %s", refreshTokenRecord.UserID)
...
func Logout(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid logout input: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input", "details": err.Error()})
		return
	}

	config.Logger.Sugar().Infof("Logout attempt with token: %s", input.RefreshToken[:8]+"...")

	// Validate the refresh token
	refreshTokenRecord, err := util.ValidateRefreshToken(input.RefreshToken)
	if err != nil {
		// Even if token is invalid, we return success for security
		config.Logger.Sugar().Warnf("Logout attempt with invalid token: %s", input.RefreshToken[:8]+"...")
	...
	if err := util.RevokeRefreshToken(refreshTokenRecord.ID); err != nil {
		config.Logger.Sugar().Errorf("Failed to revoke refresh token %s: %v", refreshTokenRecord.ID, err)
		// Don't return error to user as the token is effectively invalidated
	}

	config.Logger.Sugar().Infof("User logged out successfully: %s", refreshTokenRecord.UserID)
	c.JSON(http.StatusOK, LogoutResponse{
		Message: "Logged out successfully",
	})
}
