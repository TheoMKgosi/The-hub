...
func GetBudgets(c *gin.Context) {
	...
	config.Logger.Sugar().Infof("Fetching budgets for user ID: %s with order: %s", userIDUUID, orderClause)
	if err := config.GetDB().Where("user_id = ?", userIDUUID).Preload("Category").Order(orderClause).Find(&budgets).Error; err != nil {
		config.Logger.Sugar().Errorf("Error fetching budgets for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch budgets"})
		return
	}

	config.Logger.Sugar().Infof("Found %d budgets for user ID %s", len(budgets), userIDUUID)
...
func GetBudget(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget ID param: %s", budgetIDStr)
	...
	config.Logger.Sugar().Infof("Fetching budget ID: %s for user ID: %s", budgetID, userIDUUID)
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", budgetID, userIDUUID).Preload("Category").First(&budget).Error; err != nil {
		config.Logger.Sugar().Errorf("Budget ID %s not found for user %s: %v", budgetID, userIDUUID, err)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget not found"})
		return
	}

	config.Logger.Sugar().Infof("Successfully retrieved budget ID %s for user %s", budgetID, userIDUUID)
...
func CreateBudget(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid budget input: %v", err)
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget creation")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid start date format: %s", input.StartDate)
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid end date format: %s", input.EndDate)
	...
	if endDate.Before(startDate) {
		config.Logger.Sugar().Warnf("End date %s is before start date %s", input.EndDate, input.StartDate)
	...
	config.Logger.Sugar().Infof("Creating budget for user %s: category %s, amount %.2f", userIDUUID, input.CategoryID, input.Amount)
	if err := config.GetDB().Create(&budget).Error; err != nil {
		config.Logger.Sugar().Errorf("Error creating budget for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not create budget"})
		return
	}

	config.Logger.Sugar().Infof("Successfully created budget ID %s for user %s", budget.ID, userIDUUID)
...
func UpdateBudget(c *gin.Context) {
	...
	budgetID, err := uuid.Parse(budgetIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget ID param for update: %s", budgetIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget update")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", budgetID, userIDUUID).First(&budget).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget not found for update: ID %s, User %s", budgetID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget not found"})
		return
	}
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid update input for budget ID %d: %v", budgetID, err)
	...
	if input.StartDate != nil {
		startDate, err := time.Parse(layout, *input.StartDate)
		if err != nil {
			config.Logger.Sugar().Warnf("Invalid start date format in update: %s", *input.StartDate)
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid start date format. Use YYYY-MM-DD"})
			return
		}
	...
	if input.EndDate != nil {
		endDate, err := time.Parse(layout, *input.EndDate)
		if err != nil {
			config.Logger.Sugar().Warnf("Invalid end date format in update: %s", *input.EndDate)
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid end date format. Use YYYY-MM-DD"})
			return
		}
	...
	if input.StartDate != nil && input.EndDate != nil {
		...
		if endDate.Before(startDate) {
			config.Logger.Sugar().Warnf("End date %s is before start date %s in update", *input.EndDate, *input.StartDate)
			c.JSON(http.StatusBadRequest, gin.H{"error": "End date must be after start date"})
			return
		}
	}
	...
	if len(updates) == 0 {
		config.Logger.Sugar().Warnf("No valid fields provided for budget update: ID %d", budgetID)
		c.JSON(http.StatusBadRequest, gin.H{"error": "No valid fields to update"})
		return
	}

	config.Logger.Sugar().Infof("Updating budget ID %d for user %v with data: %+v", budgetID, userID, updates)
	if err := config.GetDB().Model(&budget).Updates(updates).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to update budget ID %d: %v", budgetID, err)
	...
	if err := config.GetDB().First(&budget, budget.ID).Error; err != nil {
		config.Logger.Sugar().Errorf("Error retrieving updated budget ID %d: %v", budget.ID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not reload updated budget"})
		return
	}

	config.Logger.Sugar().Infof("Successfully updated budget ID %d for user %v", budget.ID, userID)
...
func DeleteBudget(c *gin.Context) {
	...
	budgetID, err := uuid.Parse(budgetIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid budget ID param for delete: %s", budgetIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid budget ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during budget deletion")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", budgetID, userIDUUID).First(&budget).Error; err != nil {
		config.Logger.Sugar().Warnf("Budget not found for delete: ID %s, User %s", budgetID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Budget not found"})
		return
	}

	config.Logger.Sugar().Infof("Deleting budget ID %s for user %s", budgetID, userIDUUID)
	if err := config.GetDB().Delete(&budget).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to delete budget ID %s: %v", budgetID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete budget"})
		return
	}

	config.Logger.Sugar().Infof("Successfully deleted budget ID %s for user %s", budgetID, userIDUUID)
...
func GetBudgetAnalytics(c *gin.Context) {
	...
	if err := config.GetDB().Where("user_id = ?", userIDUUID).Preload("Category").Find(&budgets).Error; err != nil {
		config.Logger.Sugar().Errorf("Error fetching budgets for analytics: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch budgets"})
		return
	}
	...
	for _, budget := range budgets {
		...
		if err != nil {
			config.Logger.Sugar().Errorf("Error calculating spending for budget %s: %v", budget.ID, err)
			continue
		}
	...
	config.Logger.Sugar().Infof("Generated analytics for %d budgets for user %s", len(analytics), userIDUUID)
...
func GetBudgetSuggestions(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Errorf("Error generating budget suggestions for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not generate suggestions"})
		return
	}

	config.Logger.Sugar().Infof("Generated budget suggestions for user %s", userIDUUID)
...
func GetBudgetAlerts(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Errorf("Error generating budget alerts for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not generate alerts"})
		return
	}

	config.Logger.Sugar().Infof("Generated %d budget alerts for user %s", len(alerts), userIDUUID)
