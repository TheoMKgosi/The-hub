...
func GetTags(c *gin.Context) {
	...
	config.Logger.Sugar().Infof("Fetching tags for user ID: %s with order: %s", userIDUUID, orderClause)
	if err := config.GetDB().Where("user_id = ?", userIDUUID).Order(orderClause).Find(&tags).Error; err != nil {
		config.Logger.Sugar().Errorf("Error fetching tags for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch tags"})
		return
	}

	config.Logger.Sugar().Infof("Found %d tags for user ID %s", len(tags), userIDUUID)
...
func GetTag(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid tag ID param: %s", tagIDStr)
	...
	config.Logger.Sugar().Infof("Fetching tag ID: %s for user ID: %s", tagID, userIDUUID)
	var tag models.Tag
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", tagID, userIDUUID).First(&tag).Error; err != nil {
		config.Logger.Sugar().Errorf("Tag ID %s not found for user %s: %v", tagID, userIDUUID, err)
		c.JSON(http.StatusNotFound, gin.H{"error": "Tag not found"})
		return
	}

	config.Logger.Sugar().Infof("Successfully retrieved tag ID %s for user %s", tagID, userIDUUID)
...
func CreateTag(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid tag input: %v", err)
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during tag creation")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("name = ? AND user_id = ?", input.Name, userIDUUID).First(&existingTag).Error; err == nil {
		config.Logger.Sugar().Warnf("Tag name '%s' already exists for user %s", input.Name, userIDUUID)
	...
	config.Logger.Sugar().Infof("Creating tag for user %s: %s", userIDUUID, input.Name)
	if err := config.GetDB().Create(&tag).Error; err != nil {
		config.Logger.Sugar().Errorf("Error creating tag for user %s: %v", userIDUUID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not create tag"})
		return
	}

	config.Logger.Sugar().Infof("Successfully created tag ID %s for user %s", tag.ID, userIDUUID)
...
func UpdateTag(c *gin.Context) {
	...
	tagID, err := uuid.Parse(tagIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid tag ID param for update: %s", tagIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid tag ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during tag update")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", tagID, userIDUUID).First(&tag).Error; err != nil {
		config.Logger.Sugar().Warnf("Tag not found for update: ID %s, User %s", tagID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Tag not found"})
		return
	}
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid update input for tag ID %d: %v", tagID, err)
	...
	if len(updates) == 0 {
		config.Logger.Sugar().Warnf("No valid fields provided for tag update: ID %d", tagID)
		c.JSON(http.StatusBadRequest, gin.H{"error": "No valid fields to update"})
		return
	}

	config.Logger.Sugar().Infof("Updating tag ID %s for user %s with data: %+v", tagID, userIDUUID, updates)
	if err := config.GetDB().Model(&tag).Updates(updates).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to update tag ID %d: %v", tagID, err)
	...
	if err := config.GetDB().First(&tag, tag.ID).Error; err != nil {
		config.Logger.Sugar().Errorf("Error retrieving updated tag ID %d: %v", tag.ID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not reload updated tag"})
		return
	}

	config.Logger.Sugar().Infof("Successfully updated tag ID %s for user %s", tag.ID, userIDUUID)
...
func DeleteTag(c *gin.Context) {
	...
	tagID, err := uuid.Parse(tagIDStr)
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid tag ID param for delete: %s", tagIDStr)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid tag ID"})
		return
	}
	...
	userID, exist := c.Get("userID")
	if !exist {
		config.Logger.Sugar().Warn("userID not found in context during tag deletion")
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
		return
	}
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", tagID, userIDUUID).First(&tag).Error; err != nil {
		config.Logger.Sugar().Warnf("Tag not found for delete: ID %s, User %s", tagID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Tag not found"})
		return
	}
	...
	if err := config.GetDB().Table("task_tags").Where("tag_id = ?", tagID).Count(&taskCount).Error; err != nil {
		config.Logger.Sugar().Errorf("Error checking tag usage for tag ID %d: %v", tagID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not verify tag usage"})
		return
	}

	if taskCount > 0 {
		config.Logger.Sugar().Warnf("Attempted to delete tag ID %d which is used by %d tasks", tagID, taskCount)
	...
	config.Logger.Sugar().Infof("Deleting tag ID %s for user %s", tagID, userIDUUID)
	if err := config.GetDB().Delete(&tag).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to delete tag ID %d: %v", tagID, err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete tag"})
		return
	}

	config.Logger.Sugar().Infof("Successfully deleted tag ID %s for user %s", tagID, userIDUUID)
