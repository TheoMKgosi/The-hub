...
func themeBelongsToUser(db *gorm.DB, userID uuid.UUID, themeID uuid.UUID) bool {
	...
	if err := db.Model(&models.Theme{}).Where("id = ? AND user_id = ?", themeID, userID).Count(&count).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to validate theme ownership: %v", err)
		return false
	}
	return count > 0
}
...
func GetThemes(c *gin.Context) {
	...
	if err := config.GetDB().Where("user_id = ?", userIDUUID).Order("created_at ASC").Find(&themes).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to fetch themes for user %s: %v", userIDUUID, err)
...
func GetTheme(c *gin.Context) {
	...
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid theme ID param: %s", c.Param("ID"))
	...
	if err := config.GetDB().Where("id = ? AND user_id = ?", themeID, userIDUUID).First(&theme).Error; err != nil {
		config.Logger.Sugar().Warnf("Theme ID %s not found for user %s: %v", themeID, userIDUUID, err)
...
func CreateTheme(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid theme input: %v", err)
	...
	if err := config.GetDB().Create(&theme).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to create theme for user %s: %v", userIDUUID, err)
...
func UpdateTheme(c *gin.Context) {
	themeID, err := uuid.Parse(c.Param("ID"))
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid theme ID param for update: %s", c.Param("ID"))
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid theme ID"})
		return
	}
	...
	var theme models.Theme
	if err := config.GetDB().Where("id = ? AND user_id = ?", themeID, userIDUUID).First(&theme).Error; err != nil {
		config.Logger.Sugar().Warnf("Theme not found for update: ID %s, User %s", themeID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Theme not found"})
		return
	}
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		config.Logger.Sugar().Warnf("Invalid update input for theme ID %s: %v", themeID, err)
	...
	if err := config.GetDB().Model(&theme).Updates(updates).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to update theme ID %s: %v", themeID, err)
	...
	if err := config.GetDB().First(&theme, themeID).Error; err != nil {
		config.Logger.Sugar().Errorf("Error retrieving updated theme ID %s: %v", themeID, err)
...
func DeleteTheme(c *gin.Context) {
	themeID, err := uuid.Parse(c.Param("ID"))
	if err != nil {
		config.Logger.Sugar().Warnf("Invalid theme ID param for delete: %s", c.Param("ID"))
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid theme ID"})
		return
	}
	...
	var theme models.Theme
	if err := config.GetDB().Where("id = ? AND user_id = ?", themeID, userIDUUID).First(&theme).Error; err != nil {
		config.Logger.Sugar().Warnf("Theme not found for delete: ID %s, User %s", themeID, userIDUUID)
		c.JSON(http.StatusNotFound, gin.H{"error": "Theme not found"})
		return
	}

	if err := config.GetDB().Delete(&theme).Error; err != nil {
		config.Logger.Sugar().Errorf("Failed to delete theme ID %s: %v", themeID, err)
