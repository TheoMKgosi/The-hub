...
func GetTaskLearning(c *gin.Context) {
	...
	taskLearningID, err := strconv.Atoi(taskLearningIDStr)
	if err != nil {
	...
	if taskLearning.Topic.UserID != userID.(uint) {
		config.Logger.Warnf("User %v attempted to access task learning %d owned by user %d", userID, taskLearningID, taskLearning.Topic.UserID)
...
type CreateTaskLearningRequest struct {
	TopicID uint   `json:"topic_id" binding:"required" example:"1"`
...
func UpdateTaskLearning(c *gin.Context) {
	...
	// Verify user owns the topic that contains this task learning
	if taskLearning.Topic.UserID != userIDUint {
		config.Logger.Warnf("User %d attempted to update task learning %d owned by user %d", userIDUint, taskLearningID, taskLearning.Topic.UserID)
		c.JSON(http.StatusForbidden, gin.H{"error": "Access denied"})
		return
	}
...
func DeleteTaskLearning(c *gin.Context) {
	...
	// Verify user owns the topic that contains this task learning
	if taskLearning.Topic.UserID != userIDUint {
		config.Logger.Warnf("User %d attempted to delete task learning %d owned by user %d", userIDUint, taskLearningID, taskLearning.Topic.UserID)
		c.JSON(http.StatusForbidden, gin.H{"error": "Access denied"})
		return
	}

	config.Logger.Infof("Deleting task learning ID %d for user %d", taskLearningID, userIDUint)
